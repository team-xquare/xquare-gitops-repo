...
aws ecr get-login-password --region ap-northeast-2 | docker login --username AWS --password-stdin 786584124104.dkr.ecr.ap-northeast-2.amazonaws.com

docker build -t "${IMAGE_REGISTRY}/${REPO_NAME}:${IMAGE_TAG}" .

docker push "${IMAGE_REGISTRY}/${REPO_NAME}:${IMAGE_TAG}"

export $(SERVICE_NAME)_REPOSITORY="${IMAGE_REGISTRY}/${REPO_NAME}:${IMAGE_TAG}" > build_result.env